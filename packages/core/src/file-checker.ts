/**
 * File access checker - validates files against allowed/blocked patterns
 */

import { minimatch } from 'minimatch';
import type { ActiveTask, FileCheckResult } from './types.js';

export class FileChecker {
  private task: ActiveTask;

  constructor(task: ActiveTask) {
    this.task = task;
  }
  ...
  /**
   * Get summary of allowed/blocked patterns
   */
  getSummary(): { allowed: string[]; blocked: string[] } {
    return {
      allowed: this.task.allowed_files,
      blocked: this.task.blocked_files,
    };
  }
}
